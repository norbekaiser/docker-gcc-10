...
RUN DEBIAN_FRONTEND=noninteractiv apt-get install -y --no-install-recommends wget lsb_release
...
RUN DEBIAN_FRONTEND=noninteractiv sh -c 'echo deb https://packages.norbert-ruehl.de/debian $(lsb_release -cs) main > /etc/apt/sources.list.d/packages.norbert-ruehl.de.list'
